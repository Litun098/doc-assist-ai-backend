...
class RAGService:
    ...
    def get_vector_store(self, user_id: str) -> Optional[WeaviateVectorStore]:
        """
        ...
        Returns:
            WeaviateVectorStore instance or None if Weaviate is not configured
        """
    ...
    def get_query_engine(self, user_id: str, file_ids: Optional[List[str]] = None) -> Optional[RetrieverQueryEngine]:
        ...
        Returns:
            RetrieverQueryEngine instance or None if no vector store is available
        """
        # Create a cache key
    ...
    def get_chat_engine(self, user_id: str, file_ids: Optional[List[str]] = None) -> Optional[ContextChatEngine]:
        ...
        Returns:
            ContextChatEngine instance or None if no vector store is available
        """
        # Create a cache key
        cache_key = f"{user_id}_{','.join(file_ids) if file_ids else 'all'}"
    ...
    async def query_documents(self, query: str, user_id: str, file_ids: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if not query_engine:
                return {
                    "response": "Document retrieval system is not available. Please check your configuration.",
                    "query": query,
    ...
    async def chat_with_documents(self, 
        ...
        try:
            ...
            if not chat_engine:
                return {
                    "response": "Document chat system is not available. Please check your configuration.",
                    "message": message,
                    "file_ids": file_ids or [],
                    "user_id": user_id,
                    "timestamp": datetime.now().isoformat(),
